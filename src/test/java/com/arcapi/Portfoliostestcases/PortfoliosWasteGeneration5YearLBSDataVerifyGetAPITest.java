...
public class PortfoliosWasteGeneration5YearLBSDataVerifyGetAPITest extends BaseClass {
	
	@Test
	public void PortfoliosWasteGeneration5YearLBSDataVerifyGetAPI()throws IOException {
	
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.get("/portfolios/ID:" + data.getCellData(sheetName, "PortfolioID", rowNumTwo) + "/analytics/waste/?period=5").then()
				.extract().response();
		
		CommonMethod.responsetime = CommonMethod.res.getTimeIn(TimeUnit.MILLISECONDS);

		CommonMethod.test = CommonMethod.extent
				.startTest("Portfolios Waste Generation 60 Months Data GetAPITest API Test" + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Portfolios")
