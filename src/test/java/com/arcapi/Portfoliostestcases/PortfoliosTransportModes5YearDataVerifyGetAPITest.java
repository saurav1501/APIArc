...
public class PortfoliosTransportModes5YearDataVerifyGetAPITest extends BaseClass {
	...
	public void PortfoliosTransportModes5YearDataVerifyGetAPI()throws IOException {
		...
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).headers(headerMap)
				...
				.get("/portfolios/ID:" + data.getCellData(sheetName, "PortfolioID", rowNumTwo) + "/analytics/transport/?period=5").then()
				.extract().response();
		
		CommonMethod.responsetime = CommonMethod.res.getTimeIn(TimeUnit.MILLISECONDS);

		CommonMethod.test = CommonMethod.extent
				.startTest("Portfolios 5 Year Avg Distance Travel Graph Verify Get API Test" + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Portfolios")
