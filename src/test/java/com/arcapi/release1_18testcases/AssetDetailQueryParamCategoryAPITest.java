...
public class AssetDetailQueryParamCategoryAPITest extends BaseClass {

	@Test//(dependsOnMethods = { "com.arcapi.testcases.CreateAssetPOSTAPITest.CreateAssetPOSTAPI" })
	@Parameters({ "SheetName","ProjectTypeColumn","rownumber" })
	public void AssetDetailQueryParamCategoryAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		String val[] = {"energy","water","waste","transport","human_experience","regular","visitor"};
		
		for(String Category : val) {

		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/?category="+Category).then()
				.extract().response();
