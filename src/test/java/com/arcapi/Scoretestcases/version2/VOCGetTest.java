...
public class VOCGetTest extends BaseClass {
	...
	public void VOCGet(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException, ParseException {
		...
		if(obj.size()>0){
		     for (Object user : obj) {            
		         ...
		         if(!(jsonrow.get("reading")==null)) {
			         ...
			         data.setCellData("Score", "VOCReading", rownumber, reading1);
			         rownumber++;
			         }
			         else {
		...
		/* for (int i = 0; i < obj.length() ; i++)
		    {
		        JSONObject arrObj = obj.getJSONObject(i);
		...
		CommonMethod.fetchedID = CommonMethod.res.path("reading["+i+"]").toString();
		
		data.setCellData("Score", "VOCReading", i, CommonMethod.fetchedID);
		
		} */
		
		//CommonMethod.filewriteResponse(CommonMethod.ResponseJsonfile, response);
