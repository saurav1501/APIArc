...
public class PaymentOrderSubmitPOSTEmissionTest extends BaseClass {

	@Test // (dependsOnMethods = {
			// "com.arcapi.testcases.CreateAssetPOSTAPITest.CreateAssetPOSTAPI" })
	@Parameters({ "SheetName","CustomSheetName", "ProjectTypeColumn", "rownumber", "PaymentType" })
	public void PaymentOrderSubmitPOSTAPI(String SheetName,String CustomSheetName, String ProjectTypeColumn, int rownumber, String PaymentType)
			throws IOException {
...

		{
			...
			CommonMethod.res = given().log().all().parameters("name", "Test name", "email", "test-05@gmail.com",
					"unit_price", "0.0", "material_code", data.getCellData(SheetName, "MaterialCode", rownumber),
					"material_description", data.getCellData(SheetName, "MaterialDescription", rownumber), "quantity",
					"1.0", "company_code", data.getCellData(SheetName, "CompanyCode", rownumber), "street", "Test Add",
					"city", "Test city", "state", "NA", "country", "TG", "zip_code", "22673", "SoReference",
					"REGISTRATION", "actual_price", "0.0", "annual_price", "0.0", "paymetric_r",
					...
					.spec(reqSpec).when().post("/assets/LEED:"
							+ data.getCellData(CustomSheetName, ProjectTypeColumn, rownumber) + "/payments/order/")
			...
			CommonMethod.res = given().log().all()
					.parameters("name", "Test name", "email", "test-05@gmail.com", "unit_price", "0.0", "material_code",
							data.getCellData(SheetName, "MaterialCode", rownumber), "material_description",
							data.getCellData(SheetName, "MaterialDescription", rownumber), "quantity", "1.0",
							"company_code", data.getCellData(SheetName, "CompanyCode", rownumber), "street", "Test Add",
							"city", "Test city", "state", "NA", "country", "TG", "zip_code", "22673", "SoReference",
							"REGISTRATION", "actual_price", "0.0", "annual_price", "0.0")
					.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).header("Authorization", header)
					.spec(reqSpec).when().post("/assets/LEED:"
							+ data.getCellData(CustomSheetName, ProjectTypeColumn, rownumber) + "/payments/order/")
					.then().extract().response();
