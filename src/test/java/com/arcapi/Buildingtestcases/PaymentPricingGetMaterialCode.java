...
public class PaymentPricingGetMaterialCode extends BaseClass {
	...
	public void RegistrationPaymentPricingGetAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.GeneratingAuthCode("Test",111);
		...
		CommonMethod.res = given().log().all()
				...
				.get("/assets/LEED:"
						+ "1000145595"
						+ "/payments/price/?soreference=performance score").then().extract()
				.response();
		...
		//data.setCellData("Material", "MaterialCodePerformance", i, CommonMethod.fetchedID);
