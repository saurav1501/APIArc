...
public class VerifyOzoneGraphValuesAPITest extends BaseClass {
	...
	public void VerifyOzoneGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("VerifyOzoneGraphValuesAPI",
       ...
       String Ozone_Meter_ID = data.getCellData("Graphs", "TransitHEMeterID", rownumber+1);
		...
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+Ozone_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			    ...
			    //data.setCellData("Graphs", "Ozone", j+2 , reading);
