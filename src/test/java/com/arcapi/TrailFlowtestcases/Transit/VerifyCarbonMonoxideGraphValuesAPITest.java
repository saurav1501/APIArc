...
public class VerifyCarbonMonoxideGraphValuesAPITest extends BaseClass {
	...
	public void VerifyCarbonMonoxideGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("VerifyCarbonMonoxideGraphValuesAPI",
       ...
       String CO_Meter_ID = data.getCellData("Graphs", "TransitHEMeterID", rownumber);
		...
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+CO_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			   ...
			   // data.setCellData("Graphs", "CarbonMonoxide", j+2 , reading);
