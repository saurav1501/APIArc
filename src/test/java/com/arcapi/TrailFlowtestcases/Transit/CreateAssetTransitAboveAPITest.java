...
public class CreateAssetTransitAboveAPITest extends BaseClass {
	...
	public void CreateAssetPOSTAPI(String SheetName, String ProjectType,String ProjectTypeColumn, int rownumber, String Country) throws IOException {
		...
		String ProjectName = "MachineTestProject-Transit-Above-" + Country;
		
		data.setCellData(SheetName, "ProjectName", rownumber, ProjectName);
		
		data.setCellData(SheetName, "Address", rownumber, "1 mount road");
			
	    data.setCellData(SheetName, "OwnerOrg", rownumber, "A & A Dealers");
		...
		CommonMethod.res = given().log().all()
				...
				.header("Authorization", header).spec(reqSpec).when().post("/assets/registration/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Create New Transit Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Add asset")
