...
public class VerifyRidershipGraphValuesAPITest extends BaseClass {
	...
	public void VerifyRidershipGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("VerifyRidershipGraphValuesAPI",
       ...
       String Ridership_Meter_ID = data.getCellData("Graphs", "TransitHEMeterID", rownumber+7);
			
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+Ridership_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			   ...
			   // data.setCellData("Graphs", "Ridership", j+2 , reading);
