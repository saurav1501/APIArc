...
public class VerifyTVOCGraphValuesAPITest extends BaseClass {

	@Test
	@Parameters({ "SheetName","ProjectTypeColumn","rownumber" })
	public void VerifyTVOCGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("VerifyTVOCGraphValuesAPI",
       ...
       String TVOC_Meter_ID = data.getCellData("DataInput", "ExcelTemplateMeterID", rownumber+2);
			
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+TVOC_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			    ...
			    //data.setCellData("Graphs", "TVOC", j+2 , reading);
				String Act_Reading = data.getCellData("Graphs", "TVOC", j+2);
