...
public class VerifyWasteGraphValuesAPITest extends BaseClass {
	...
	public void VerifyEnergyGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
       ...
       String Energy_Meter_ID = data.getCellData("DataInput", "ExcelTemplateMeterID", rownumber);
       
       String Energy_Meter_Name = data.getCellData("DataInput", "ExcelTemplateMeterName", rownumber);
       
       CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Content-type", "application/json").header("Authorization", header).spec(reqSpec)
				.when()
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/waste/")
				.then().extract().response();
       CommonMethod.res.then().spec(respSpec);
       System.out.println(CommonMethod.res.asString());
      
       CommonMethod.fetchedID = CommonMethod.res.path("results.id[0]").toString();
	
		data.setCellData("Graphs", "WasteID", rownumber , CommonMethod.fetchedID);
       
     /* String Unit[] = {"lbs","kg","tons"};
       end_date: "2018-10-01"
...
waste_generated: 200
       for (String unit : Unit) {*/
		...
		jsonAsMap.put("end_date", "2017-09-01");
		jsonAsMap.put("start_date", "2018-10-01");
		jsonAsMap.put("unit", "lbs");
		/*jsonAsMap.put("waste_diverted", "29");
		jsonAsMap.put("waste_generated", "29");*/
		
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Content-type", "application/json").header("Authorization", header).spec(reqSpec)
				.body(jsonAsMap).when()
				.put("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/waste/ID:"
						+ CommonMethod.fetchedID + "/")
				.then().extract().response();
