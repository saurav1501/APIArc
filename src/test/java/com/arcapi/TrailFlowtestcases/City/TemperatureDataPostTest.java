...
public class TemperatureDataPostTest extends BaseClass {
	...
	public void TemperatureDataPost(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		jsonAsMap.put("start_date", "2017-02-01T00:00:00");
		jsonAsMap.put("end_date", "2017-02-02T00:00:00");

		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("content-type", "application/json").header("Authorization", header).spec(reqSpec)
				.body(jsonAsMap).when()
				.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/temperature/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("TemperatureData Post API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
