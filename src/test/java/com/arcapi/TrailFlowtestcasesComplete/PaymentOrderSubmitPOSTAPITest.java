...
public class PaymentOrderSubmitPOSTAPITest extends BaseClass {
	...
	@Parameters({ "SheetName", "ProjectTypeColumn", "rownumber", "PaymentType", "Term" })
	public void PaymentOrderSubmitPOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber, String PaymentType, int Term)
			throws IOException {
...

		{
			...
			CommonMethod.res = given().log().all().parameters("name", "Test name", "email", "test-05@gmail.com",
					 "material_code", data.getCellData(SheetName, "MaterialCode", rownumber),
					...
					"REGISTRATION", "term", Term, "paymetric_r",
					...
					.spec(reqSpec).when().post("/assets/LEED:"
							+ data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/payments/order/")
			...
			CommonMethod.res = given().log().all()
					.parameters("name", "Test name", "email", "test-05@gmail.com", "material_code",
