...
public class VerifySulphurDioxideGraphValuesAPITest extends BaseClass {
	...
	public void VerifySulphurDioxideGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("VerifySulphurDioxideGraphValuesAPI",
       ...
       String SO_Meter_ID = data.getCellData("Graphs", "TransitHEMeterID", rownumber+3);
			
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+SO_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			    ...
			    //data.setCellData("Graphs", "SulphurDioxide", j+2 , reading);
