...
public class PortfoliosTeamsPUTAPITest extends BaseClass {
	...
	public void PortfoliosTeamsPUTAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		jsonAsMap.put("permission", "can_write");
		...
		CommonMethod.res = given().log().all()
				...
				.body(jsonAsMap)
				.when()
				.put("/portfolios/ID:"+data.getCellData(SheetName, "PortfolioID_StartClient", rownumber)+"/teams/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent.startTest(
				"PortfoliosTeams PUT API Test  "+ CommonMethod.getLabel(CommonMethod.responsetime),
