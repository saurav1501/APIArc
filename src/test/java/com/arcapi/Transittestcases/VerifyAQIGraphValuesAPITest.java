...
public class VerifyAQIGraphValuesAPITest extends BaseClass {
	...
	public void VerifyAQIGraphValuesAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
       ...
       String AQI_Meter_ID = data.getCellData("Graphs", "TransitHEMeterID", rownumber+6);
		...
		CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/resampled/"+AQI_Meter_ID+"/").then()
				.extract().response();
		...
		System.out.println(CommonMethod.res.asString());
		 ...
		 for(int j=0;j<12;j++) {
			    ...
			    //data.setCellData("Graphs", "AQI", j+2 , reading);
