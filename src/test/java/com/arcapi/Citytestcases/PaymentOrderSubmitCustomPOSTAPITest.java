...
public class PaymentOrderSubmitCustomPOSTAPITest extends BaseClass {

	@Test//(dependsOnMethods = { "com.arcapi.testcases.CreateAssetPOSTAPITest.CreateAssetPOSTAPI" })
	@Parameters({ "SheetName", "CustomSheetName", "rownumber","PaymentType"})
	public void PaymentOrderSubmitPOSTAPI(String SheetName, String CustomSheetName, int rownumber, String PaymentType)
			throws IOException {
		...
		CommonMethod.res = given().log().all()
				...
				.post("/assets/LEED:" + Customdata.getCellData(CustomSheetName, "ProjectID", i) + "/payments/order/")
			...
			CommonMethod.res = given().log().all()
					...
					.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("Authorization", header).spec(reqSpec).when()
					.post("/assets/LEED:" + data.getCellData(CustomSheetName, "ProjectID", i) + "/payments/order/")
					.then().extract().response();
