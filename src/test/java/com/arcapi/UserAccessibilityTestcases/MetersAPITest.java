...
public class MetersAPITest extends BaseClass {
	...
	public void MetersAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/meters/").then()
				.extract().response();
		...
		CommonMethod
				.fetchingJSONResponse("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/meters/");
