...
public class CountryStatesJSONAPITest extends BaseClass {
	...
	public void CountryStatesJSONAPI(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/country/states/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Country States Json API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies list of countries and states")
				.assignCategory("CheckCountryStates");
		
		CommonMethod.testlog("Info", "Response from API" + CommonMethod.res.asString() );
        CommonMethod.testlog("Info","Content Type is : " + CommonMethod.res.getContentType());
        CommonMethod.testlog("Info","Status Code is : " + CommonMethod.res.getStatusCode());

		CommonMethod.fetchingJSONResponse("/country/states/");
