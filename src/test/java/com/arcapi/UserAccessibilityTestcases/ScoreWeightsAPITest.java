...
public class ScoreWeightsAPITest extends BaseClass {
	...
	public void ScoreWeightsAPI(String SheetName,int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/weights/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Score Weights API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies score weights")
				.assignCategory("CheckScore");
		CommonMethod.testlog("Info", "Response from API" + CommonMethod.res.asString() );
        CommonMethod.testlog("Info","Content Type is : " + CommonMethod.res.getContentType());
        CommonMethod.testlog("Info","Status Code is : " + CommonMethod.res.getStatusCode());


		CommonMethod.fetchingJSONResponse("/weights/");
