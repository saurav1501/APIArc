...
public class S3DocumentUploadPOSTAPITest extends BaseClass {
	...
	public void S3DocumentUploadPOSTAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.res = given().log().all()
				.multiPart("action_file", CommonMethod.formuploadfile)
				.multiPart("upload_category", "PF901")
				.header("Ocp-Apim-Subscription-Key",
						CommonMethod.SubscriptionKey)
				.header("Authorization", header)
				.spec(reqSpec)
				.when()
				.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/uploadS3/").then().extract()
				.response();
		System.out.println(CommonMethod.responsetime);
		
		CommonMethod.responsetime = CommonMethod.res.getTimeIn(TimeUnit.MILLISECONDS);
		
		CommonMethod.test = CommonMethod.extent.startTest("S3Document Upload POSTAPI Test  "+ CommonMethod.getLabel(CommonMethod.responsetime), "Verifies credit upload document file").assignCategory("CheckCredit");
