...
public class AssetValidationInfoPOSTAPITest extends BaseClass {
	...
	public void AssetValidationInfoPOSTAPI(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().parameters("state", "05", "country", "TH", "zip_code", "28293")
				.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when()
				.post("/assets/validation/").then().extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Asset Validation Info Post API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Validates asset Info")
