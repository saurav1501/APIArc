...
public class PortfoliosAssetDetailGetAPITest extends BaseClass {
	...
	public void PortfoliosAssetDetailGetAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.spec(reqSpec).when().get("/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber)
		...
		CommonMethod.test = CommonMethod.extent.startTest(
				"Portfolios AssetDetail Get API" + CommonMethod.getLabel(CommonMethod.responsetime),
		...
		CommonMethod.fetchingJSONResponse("/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber)
				+ "/assets/" + data.getCellData(SheetName, "PortfolioPK", rownumber) + "/");
