...
public class AssociatedCategoriesAPITest extends BaseClass {
	...
	public void AssociatedCategoriesAPI(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/categories/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Associated Categories API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Associated categories")
				.assignCategory("CheckAssociatedCategories");
		
		CommonMethod.testlog("Info", "Response from API" + CommonMethod.res.asString() );
        CommonMethod.testlog("Info","Content Type is : " + CommonMethod.res.getContentType());
        CommonMethod.testlog("Info","Status Code is : " + CommonMethod.res.getStatusCode());

		CommonMethod.fetchingJSONResponse("/categories/");
