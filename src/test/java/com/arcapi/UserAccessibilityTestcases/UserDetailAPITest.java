...
public class UserDetailAPITest extends BaseClass {
	...
	public void UserDetailAPI(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/auth/detail/").then().extract()
				.response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("User Detail API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies User detail")
				.assignCategory("CheckUserDetail");
