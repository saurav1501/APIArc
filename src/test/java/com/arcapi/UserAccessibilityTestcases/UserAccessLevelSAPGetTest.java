...
public class UserAccessLevelSAPGetTest extends BaseClass {
	...
	public void UserAccessLevelSAPGet(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/auth/access/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("UserAccessLevelSAP Get API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies User Access from SAP")
				.assignCategory("Useraccess");
		...
		CommonMethod.fetchingJSONResponse("/auth/access/");
