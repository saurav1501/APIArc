...
public class TemperatureDataGetTest extends BaseClass {
	...
	public void TemperatureDataGet(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/temperature/").then()
				.extract().response();
		...
		CommonMethod.fetchingJSONResponse(
				"/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/temperature/");
