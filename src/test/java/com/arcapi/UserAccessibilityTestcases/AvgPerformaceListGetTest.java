...
public class AvgPerformaceListGetTest extends BaseClass {
	...
	public void AvgPerformaceList(String SheetName, int rownumber) {
		...
		CommonMethod.res= given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/comparables/?state=USDC")
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("AvgPerformaceList API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
		...
		CommonMethod.fetchingJSONResponse("/comparables/?state=USDC");
