...
public class ThirdPartyListAppsAPITest extends BaseClass {
	...
	public void ThirdPartyListAppsAPI(String SheetName, int rownumber) {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				.header("Authorization", header).spec(reqSpec).when().get("/apps/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Third Party List Apps API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Apps")
				.assignCategory("CheckApps");
		...
		CommonMethod.fetchingJSONResponse("/apps/");
