...
public class PortfoliosTeamsGetAPITest extends BaseClass {
	...
	public void PortfoliosTeamsGetAPI(String SheetName,String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
				...
				.get("/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber) + "/teams/").then()
				.extract().response();
		...
		CommonMethod.test = CommonMethod.extent.startTest(
				"Portfolios Teams Get API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
				"Verifies TeamList of Portfolios");
		...
		CommonMethod.fetchingJSONResponse(
				"/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber) + "/teams/");
