...
public class SignAgreementPOSTAPITest extends BaseClass {
	...
	public void fetchingJsonRes(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent.startTest("Sign Agreement POST API Test", "Verifies sign Agreement")
		...
		CommonMethod.res = given().multiPart("agreement", str).parameters("SoReference", str)
				.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).header("Authorization", header)
				.spec(reqSpec).when()
				.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/agreements/")
