...
public class WasteGeneratedBulkDataPostTest extends BaseClass {
	...
	public void WasteGeneratedBulkDataPost(String SheetName, String ProjectTypeColumn, int rownumber)
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("WasteGeneratedBulkData Post API Test  ", "Post Waste data generated")
		...
		for (String str : value) {
			...
			jsonAsMap.put("reading", str);
			jsonAsMap.put("unit", str);

			JSONObject jsonAsMap1 = new JSONObject();
			jsonAsMap1.put("start_date", str);
			jsonAsMap1.put("end_date", str);
			jsonAsMap1.put("reading", str);
			jsonAsMap1.put("unit", str);
			...
			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("content-type", "application/json").header("Authorization", header).spec(reqSpec).body(list)
					.when().post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber)
							+ "/waste/generated/")
