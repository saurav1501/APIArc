...
public class GhgCalcGetTest extends BaseClass {
	...
	public void GhgCalcGet(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent.startTest("GhgCalc Get API Test  ", "Verifies List of Assets")
		...
		for (String str : value) {
			...
			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("Authorization", header).spec(reqSpec).when().get("/assets/LEED:"
							+ data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/ghgcalc/?resample=" + str)
