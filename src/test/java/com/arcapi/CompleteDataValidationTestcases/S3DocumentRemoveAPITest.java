...
public class S3DocumentRemoveAPITest extends BaseClass {
	...
	public void S3DocumentRemoveAPI(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("S3DocumentRemove API Test",
		...
		for (String str : value) {
			...
			CommonMethod.res = given().log().all().parameters("doc_id", str)
					.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).header("Authorization", header)
					.spec(reqSpec).when()
					.put("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/s3documents/")
