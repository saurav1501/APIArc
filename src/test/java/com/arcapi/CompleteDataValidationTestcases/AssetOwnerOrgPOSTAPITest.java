...
public class AssetOwnerOrgPOSTAPITest extends BaseClass {
	...
	public void AssetOwnerOrgPOSTAPI() throws IOException {
		...
		for (String str : value) {
			...
			jsonAsMap.put("orgName", str);
			jsonAsMap.put("orgContactName", str);
			jsonAsMap.put("orgContactEmail", str);
			jsonAsMap.put("orgWebSite", str);
			jsonAsMap.put("orgCategory", str);
			jsonAsMap.put("orgSubCategory", str);

			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("content-type", "application/json").header("Authorization", header).spec(reqSpec)
					.body(jsonAsMap).when().post("/assets/searchowner/").then().contentType(ContentType.JSON).extract()
					.response();
