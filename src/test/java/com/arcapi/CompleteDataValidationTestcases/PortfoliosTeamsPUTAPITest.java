...
public class PortfoliosTeamsPUTAPITest extends BaseClass {
	...
	public void PortfoliosTeamsPUTAPI(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("PortfoliosTeams PUT API Test  " + CommonMethod.getLabel(CommonMethod.responsetime),
						"Verifies Portflios")
		...
		for (String str : value) {
			...
			jsonAsMap.put("username", str);
			jsonAsMap.put("permission", str);

			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("Content-type", "application/json").header("Authorization", header).spec(reqSpec)
					.body(jsonAsMap).when()
					.put("/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber) + "/teams/").then()
					.extract().response();
