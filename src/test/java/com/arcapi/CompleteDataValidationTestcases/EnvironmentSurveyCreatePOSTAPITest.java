...
public class EnvironmentSurveyCreatePOSTAPITest extends BaseClass {
	...
	public void EnvironmentSurveyCreatePOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber)
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Environment Survey create API Test  ", "Verifies Environment Survey creation")
		...
		for (String str : value) {
			...
			jsonAsMap.put("satisfaction", str);
			jsonAsMap.put("location", str);
			jsonAsMap.put("complaints", str);
			jsonAsMap.put("other_complaint", str);

			CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("content-type", "application/json").spec(reqSpec).body(jsonAsMap).when()
					.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber)
							+ "/survey/environment/?key=" + data.getCellData(SheetName, "BuildingKeyID", rownumber))
