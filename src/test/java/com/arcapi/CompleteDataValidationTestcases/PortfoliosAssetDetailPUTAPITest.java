...
public class PortfoliosAssetDetailPUTAPITest extends BaseClass {
	...
	public void PortfoliosAssetDetailPUTAPI(String SheetName, String ProjectTypeColumn, int rownumber)
			throws IOException {
		...
		for (String str : value) {
			...
			jsonAsMap.put("asset_name", str);
			jsonAsMap.put("asset_country", str);
			jsonAsMap.put("asset_address", str);
			jsonAsMap.put("property_type", str);

			CommonMethod.res = given().log().all().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("Content-type", "application/json").header("Authorization", header).spec(reqSpec)
					.body(jsonAsMap).when()
					.put("/portfolios/ID:" + data.getCellData(SheetName, "PortfolioID", rownumber) + "/assets/"
							+ data.getCellData(SheetName, "PortfolioPK", rownumber) + "/")
