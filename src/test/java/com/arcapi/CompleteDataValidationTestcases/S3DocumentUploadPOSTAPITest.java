...
public class S3DocumentUploadPOSTAPITest extends BaseClass {
	...
	public void S3DocumentUploadPOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		for (String str : value) {
			...
			CommonMethod.res = given().log().all().multiPart("action_file", str)
					.multiPart("upload_category", str)
					.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).header("Authorization", header)
					.spec(reqSpec).when()
					.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/uploadS3/")
