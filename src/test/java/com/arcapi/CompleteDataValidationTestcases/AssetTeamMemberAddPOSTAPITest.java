...
public class AssetTeamMemberAddPOSTAPITest extends BaseClass {
	...
	public void AssetTeamMemberAddPOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber)
		...
		for (String str : value) {
			...
			CommonMethod.res = given().parameters("user_email", str, "Reltyp", str, "Responsibility", str)
					.header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey).header("Authorization", header)
					.spec(reqSpec).when()
					.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/teams/")
