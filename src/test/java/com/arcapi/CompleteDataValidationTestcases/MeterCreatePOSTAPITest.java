...
public class MeterCreatePOSTAPITest extends BaseClass {
	...
	public void MeterCreatePOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber) throws IOException {
		...
		CommonMethod.test = CommonMethod.extent.startTest("Meter Create Post API Test  ", "Verifies Meter Creation")
		...
		for (String str : value) {
			...
			jsonAsMap.put("type", str);

			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("content-type", "application/json").header("Authorization", header).spec(reqSpec)
					.body(jsonAsMap).when()
					.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/meters/")
