...
public class AnalysisDataSubmitPOSTAPITest extends BaseClass {
	...
	public void AnalysisDataSubmitPOSTAPI(String SheetName, String ProjectTypeColumn, int rownumber)
		...
		CommonMethod.test = CommonMethod.extent
				.startTest("Analysis data Post API Test  ",
		...
		for (String str : value) {
			...
			jsonAsMap.put("id", str);
			jsonAsMap.put("category", str);
			...
			jsonAsMap.put("created_at", str);
			jsonAsMap.put("effective_at", str);
			jsonAsMap.put("building", str);

			CommonMethod.res = given().header("Ocp-Apim-Subscription-Key", CommonMethod.SubscriptionKey)
					.header("content-type", "application/json").header("Authorization", header).spec(reqSpec)
					.body(jsonAsMap).when()
					.post("/assets/LEED:" + data.getCellData(SheetName, ProjectTypeColumn, rownumber) + "/analysis/")
